...
def Scrap(link):
    ...
    with open(fileName, "w", encoding='utf-8') as f:
        ...
        for child in children:
            courseOverview = courseOverview+child.text.strip()
            f.write(child.text + "\n")
        ...
        studentFee = json.dumps(
            {"HomeStudent": homeStudentFee, "InternationalStudent": internationalFee, "additionalInfo": moreFeeInfo})
        # entry requirement
        ...
        entryRequirementsToAddToSQL = "INTERNATIONAL STUDENT: " + \
            internationalStudenEnty + '\n'
        ...
        entryRequirementsToAddToSQL = entryRequirementsToAddToSQL + \
            "LOCAL STUDENT: " + locaStudentRequirement
        entryRequirement = json.dumps(
            {"LocalStudent": locaStudentRequirement, "InternationalStudent": internationalStudenEnty})
        lengthOfCourse = courseLength[0].text.strip(
            "\n").split("Course length")
        courseCodeText = courseCode.strip("\n").split("UCAS code")
     
        addDataToDB(courseName[0], lengthOfCourse[1].strip("\n"),courseCodeText[1].strip("\n"),title, description, courseModuleToInsertToSQL, studentFee.strip("\u00a39"), entryRequirement.strip("\n"))
...
if __name__ == '__main__':
    with open("undergraduate-links.txt", "r") as file:
